# Autoencoder for communications with Reconfigurable Intelligent Surfaces
...
############### Encoder
Input = tf.keras.Input(shape=(n_x), dtype=tf.float64)
model_encode = tf.keras.Sequential()
model_encode.add(Dense(units=64, use_bias = True, activation='tanh'))
model_encode.add(Dense(units = 2, use_bias = True, activation='tanh'))    # Shape: (None, 2)
...
x_encode = x_encode / tf.sqrt(tf.reduce_mean(tf.math.multiply(x_encode, x_encode)))

x_ris = model_TR(x_encode) 
...
for i in range(n_rounds):
    ...
    if i == (n_rounds-1):
      his = com_model.fit([x_train, noise_train], x_train, validation_data = ([x_test, noise_test], x_test), batch_size = batch_size, epochs=1, verbose=1)
    else:
      com_model.fit([x_train, noise_train], x_train, batch_size = batch_size, epochs=5, verbose=1)
...
for Eb_No_dB in np.arange(-2.0, 16.0, 0.5):
    ...
    decoded_sys_round = np.round(com_model.predict([x_test,noise_try]))
...
# np.savetxt('BLER_sim_8_AE_RIS.txt', BLER)
np.savetxt('BLER_sim_16_AE_RIS.txt', BLER)
...
plt.plot(x_encode_[:,0], x_encode_[:,1], 'bx', markersize=12)
